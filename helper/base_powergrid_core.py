...
class BasePowerGridCore:
    ...
    def convert(dictionary):
        ...
        """
        return namedtuple(typename='GenericDict',
                          field_names=dictionary.keys())(**dictionary)
    ...
    def _set_mpv_parameters(self):
        ...
        self.mpv_concentration_rate_percent = self.args['mpv_settings']['mpv_concentration_rate_percent']
    ...
    def _set_input_output_paths(self):
        ...
        self.output_test_path = self.args["cfg_settings"]["output_test_path"] # output_test_data_path
        self.sequence_id_path = self.args["cfg_settings"]["sequence_id_path"]

        self.input_data_type = self.args["cfg_settings"]["input_data_type"]
        self.output_file_type = self.args["cfg_settings"]["output_file_type"] # output_file_type
    ...
    def _set_simbench_episode_settings(self):
        ...
        self.valid_sb_scenario_storage = self.args["rawdata"]["valid_sb_scenario_storage"]
        # Set scenario name
    ...
    def _integrate_mpv_sgens_into_load_buses_for_MPVs(self):
        ...
        try:
            ...
            num_mpv_influenced_load_buses = int((self.mpv_concentration_rate_percent / 100) * total_loads)
            ...
            print(f"{'mpv_concentration_rate_percent:':<35}{self.mpv_concentration_rate_percent:>4.2f}%")
            print(f"{'num_mpv_influenced_load_buses:':<35}{total_loads:>2d} total loads of {num_mpv_influenced_load_buses:>2d} mpvsgens")
            print(f"{'mpv_inverter_apparent_power_mw:':<35}{self.mpv_inverter_apparent_power_mva:>2.6f} MW")
            print(f"{'mpv_solar_cell_capacity_mw:':<35}{self.mpv_solar_cell_capacity_mw:>2.6f} MW")
            ...
            for i in range(num_mpv_influenced_load_buses):
                ...
                if selected_load_bus_id in self.base_powergrid.sgen['bus'].values:
                    ...
                    sgen_at_bus = self.base_powergrid.sgen[bus_condition & name_condition]
                else:
                ...
                sgen_bes_name = f"PV{selected_sgen_bus_id}-Load{selected_load_bus_id} SGenMPV {i}"
                pp.create_sgen(net=self.base_powergrid,
                ...
                formatted_part1 = f" SGenMPV {i} added to grid in Bus Id {selected_load_bus_id}."
            ...
            name_condition = self.base_powergrid.sgen.name.str.contains("SGenMPV")
            extract_sgen_mpv_num = self.base_powergrid.sgen.name[name_condition].str.extract('(\d+)$').astype(int)
            # Count entries containing "SGenMPV":
            self.mpvsgen_num = self.base_powergrid.sgen.name.str.contains("SGenMPV").sum()
            ...
            self.mpv_range_index = pd.RangeIndex(start=pv_stop_index,
                                                 stop=mpv_stop_index,
                                                 step=mpv_step_index)
    ...
    def _integrate_sgens_into_storage_buses_for_PVBES(self):
        ...
        try:
            ...
            for i in range(len(storage_buses)):
                ...
                sgen_at_bus = self.base_powergrid.sgen[self.base_powergrid.sgen['bus'] == sgen_bus_id]
                ...
                phys_type = sgen_at_bus['phys_type'].iloc[0]
                subnet = sgen_at_bus['subnet'].iloc[0]
                sgen_bes_name = f"{i+1}: PV{sgen_bus_id}-BES{storage_bus_id} SGen {i+num_sgen}"
                pp.create_sgen(net=self.base_powergrid, 
                               bus=storage_bus_id, 
                               p_mw=p_mw, 
                               q_mvar=q_mvar, 
    ...
    def load_data_and_model(self):
        ...
        self.base_profiles = simbench.get_absolute_values(self.base_powergrid,
    ...
    def update_voltage_inverter_control_parameters(self):
        ...
        self.inverter_ctrl_params[self.regulation_standard]\
                                  [self.standard_mode]\
                                  ['v_nom_net'] = v_nom_net_value
    ...
    def calculate_and_display_reactive_power_boundaries(self):
        ...
        print(f"{'PV Load Power Ratio (PV Max / Load Max):':<35}{pv_load_ratio:>4.2f} MW")
    ...
    def load_arguments_from_yaml_configuration(self, args):
        ...
        config_filename = args.cfg_user_path.split(".")[0] + f'_{suffix_episode_limit}.yaml'
        config_output_folder_name = os.path.join(config_folder_path, config_filename)

        if args.mpv_flag:
            additional1= f"_mpv_{args.mpv_flag}_con_{int(args.mpv_concentration_rate_percent)}"
    ...
    def update_arguments_from_master_script(self, main_cfg, args):
        ...
        Args:
            ...
            args (dict): Dictionary containing the keyword arguments.
        """
        main_cfg["rawdata"].update({"benchmark": args.benchmark,
                                    "sb_code": args.sb_code,
                                    "scenario": args.scenario,
                                    "scaling": {"pv": args.scaling_pv,
                                                "load": args.scaling_load,
                                                "storage": args.scaling_storage},
                                    "soc_initial": args.soc_initial})
        main_cfg["sim"].update({"time_mode": args.time_mode,
        ...
        main_cfg["mcs_settings"].update({"seed_value": args.seed_value,
                                         "flag_monte_carlo": args.flag_monte_carlo,
                                         "num_monte_carlo_runs": args.num_monte_carlo_runs})
        main_cfg["mpv_settings"].update({"mpv_flag": args.mpv_flag,
    ...
    def save_updated_configuration(self, main_cfg):
        ...
        with open(main_cfg["cfg"]["config_output_folder_name"], 'w', encoding='utf-8') as filepath:
    ...
    def load_inverter_control_settings(self, main_cfg, args):
        ...
        if self.regulation_standard not in main_cfg["regulation_standard_modes"]:
            raise ValueError("Invalid standard. Choose 'vde' ('VDE-AR-N 4105')"
                             " or 'ieee' ('IEEE Std 1547-2018').")
        if self.standard_mode not in main_cfg["standard_modes"]:
            raise ValueError(f"Invalid standard mode. Choose one of the available modes"
                             f"in {args.cfg_user_path}.")
        # Load the inverter control parameters
        self.inverter_ctrl_params = main_cfg # [self.regulation_standard] # [standard_mode]
    ...
    def _set_control_mode(self, config, arg_ctrl_name, str_ctrl_name):
        ...
        Returns:
            None
        """
        if arg_ctrl_name is not None and arg_ctrl_name in self.valid_modes[str_ctrl_name]:
            ctrl_mode = arg_ctrl_name
        else:
            ctrl_mode = config["control_modes"][str_ctrl_name]
            print(f"----------Set control mode for a given argument----------\n"
    ...
    def _check_config_settings(self):
        ...
        """
        errors = {}
        assert self.main_cfg["sim"]["time_mode"] in ['selected', 'random'], \
        ...
        if self.main_cfg["sim"]["episode_start_min_interval"] not in range(0, 4):
            errors['start_interval_error'] = 'Specify the starting interval between 0 and 3.!'
        if self.main_cfg["sim"]["episode_limit"] not in range(0, 9600):
            errors['episode_limit_error'] = 'Specify the max. number of time steps in an episode.!'
    ...
    def select_data_with_sampling_interval(data_path, header_rows, data_interval,
                                           desired_interval, scaling_factor):
        ...
        selected_data = time_series_data.iloc[start_index::step_size, 1:] * scaling_factor
    ...
    def _load_mpv_data(self):
        ...
        if self.mpv_benchmark == "mpvbench":
            ...
            mpv_data = self.add_noise_to_data(data=mpv_data,
        ...
        # Code for loading from simbench
            ...
            adjusted_solar_capacity_mw = self.mpv_solar_cell_capacity_mw*solar_cell_efficiency_rate
            ...
            plt.plot(avg_mpv_data_timestamps, label="avg mpv data", color='red', linewidth=2)
            # Plotting the confidence interval around the average MPV data
            upper_confidence = avg_mpv_data_timestamps + avg_std_mpv_data_timestamps*0.01
            lower_confidence = avg_mpv_data_timestamps - avg_std_mpv_data_timestamps*0.01
            plt.fill_between(range(num_timestamps), 
                             lower_confidence, 
                             upper_confidence, 
                             color='red', 
                             alpha=0.1)
    ...
    def _load_pv_data(self):
        ...
        if self.benchmark == "simbench":
            ...
            # add uncertainty with unit truncated gaussian (only positive accepted)
            pv_data = self.add_noise_to_data(data=pv_data,
                                             add_noise=self.add_noise,
                                             std_dev_factor=10.0)
            if self.args["cfg_settings"]["write_to_rawdata"] is True:
                pv_data.index = pd.to_datetime(
                    self.base_powergrid.profiles['renewables']["time"])
                output_path = os.path.join(self.output_data_path,
    ...
    def _load_loads_active_power_data(self):
        ...
        with the flexibility to vary based on the raw sensor data."""
        if self.benchmark == "simbench":
            load_active_power_data = (self.base_profiles[('load', 'p_mw')] *
                                      self.scaling_load)
            # add uncertainty with unit truncated gaussian (only positive accepted)
            load_active_power_data = self.add_noise_to_data(data=load_active_power_data,
                                                            add_noise=self.add_noise,
            ...
            load_active_power_data_path = os.path.join(self.input_data_path, 'load_active.csv')
            ...
            raise TypeError(f'Oops! Invalid configuration file for benchmark: "{self.benchmark}".')
    ...
    def _load_loads_reactive_power_data(self):
        ...
        with the flexibility to vary based on the raw sensor data."""
        if self.benchmark == "simbench":
            load_reactive_power_data = (self.base_profiles[('load', 'q_mvar')] *
                                        self.scaling_load)
            # add uncertainty with unit truncated gaussian (only positive accepted)
            load_reactive_power_data = self.add_noise_to_data(data=load_reactive_power_data,
                                                              add_noise=self.add_noise,
                                                              std_dev_factor=100.0)
            if self.args["cfg_settings"]["write_to_rawdata"] is True:
            ...
            load_reactive_power_data_path = os.path.join(self.input_data_path, 'load_reactive.csv')
    ...
    def _load_time_data(self):
        ...
        if self.benchmark == "simbench":
            ...
            start_date = datetime.datetime.strptime(start_string,
                                                    format_string)
            end_date = datetime.datetime.strptime(end_string,
                                                  format_string)
            date_range = pd.date_range(start_date, end_date,
                                       freq=str(self.resolution) +'min') # minutely frequency
            ...
            if self.args["cfg_settings"]["write_to_rawdata"] is True:
                time_data.index = pd.to_datetime(
                    self.base_powergrid.profiles['renewables']["time"])
                output_path = os.path.join(self.input_data_path,
    ...
    def _load_storage_data(self):
        ...
        if self.benchmark == "simbench":
            ...
            storage_reactive_power_data = copy.deepcopy(
                self.base_profiles[('storage','p_mw')])*0
            if self.args["cfg_settings"]["write_to_rawdata"] is True:
            ...
            storage_path = os.path.join(self.input_data_path,
                                        'load_storage.csv')
    ...
    def _load_and_apply_storage_constraints(self):
        ...
        if self.is_storage_scenario:
            ...
            self.storage_active_power, self.storage_reactive_power = self._load_storage_data()
    ...
    def get_zone_data(self):
        ...
        graph = create_nxgraph(self.base_powergrid, respect_switches=True,
                               nogobuses=None,notravbuses=None)
        # predefined vars
        # branch_paths = []
        bus_distance = pd.Series(
            nx.single_source_dijkstra_path_length(graph,
                                                  source_node,
                                                  weight="weight"))
        zone_data = {}
        iteration_count = 0
        ...
        while not (bus_distance.values == 0).all():
            ...
            self.base_powergrid.bus.loc[selected_rows, 'zone'] = f'zone{iteration_count+1}'
            bus_distance = pd.Series(
                nx.single_source_dijkstra_path_length(graph, source_node, weight="weight"))
            iteration_count +=1
        zone_data["main"] = list(graph.nodes)
        self.base_powergrid.bus.loc[self.base_powergrid.bus.index.isin(zone_data["main"]),
                                    'zone'] = 'main'
    ...
    def _get_penetration_rates(self):
        ...
        part_load_reactive_data = self.load_reactive_power_data.iloc[start_step:end_step]
        ...
        apparent_load_power_sim_sum = np.sqrt(load_active_sim_sum**2 + load_reactive_sim_sum**2)
    
        # Calculating penetration rates
        pv_penetration_rate = pv_sim_sum / load_active_sim_sum if load_active_sim_sum != 0 else 0
        pv_penetration_rate2 = pv_sim_sum / apparent_load_power_sim_sum if apparent_load_power_sim_sum != 0 else 0
        ...
        if self.mpv_flag:
            ...
            mpv_penetration_rate = mpv_sim_sum / load_active_sim_sum if load_active_sim_sum != 0 else 0
            mpv_penetration_rate2 = mpv_sim_sum / apparent_load_power_sim_sum if apparent_load_power_sim_sum != 0 else 0
    
        # Printing the results
        print("-------------------  Penetration PV\MPV  -------------------")
        print(f"{'Total Apparent Power Load (S) in Simulation:':<60}{apparent_load_power_sim_sum:.2f} MVA")
        print(f"{'Total Active Power Load (P) in Simulation:':<60}{load_active_sim_sum:.2f} MW")
        print(f"{'Total Reactive Power Load (Q) in Simulation:':<60}{load_reactive_sim_sum:.2f} MVar")
        print(f"{'Total Active Power Generation from PV (P) in Simulation:':<60}{pv_sim_sum:.2f} MW")
        print(f"{'Total Active Power Generation from MPV (P) in Simulation:':<60}{mpv_sim_sum:.2f} MW")
    ...
    def _create_rootnet(self, root_net):
        ...
        """
        if root_net is None:
            raise TypeError("Please provide a base powergrid configured as pandapower format.")
        return root_net
   ...
   # bus
    ...
    def get_res_bus_voltage_angle(self):
        ...
        res_bus_voltage_angle = self.net.res_bus.va_degree.sort_index().to_numpy(copy=True)
    ...
    def get_res_line_loading(self):
        ...
        res_line_loading = self.net.res_line.loading_percent.sort_index().to_numpy(copy=True)
    ...
    def get_res_trafo_loading(self):
        ...
        res_trafo_loading = self.net.res_trafo.loading_percent.sort_index().to_numpy(copy=True)
    ...
    def get_res_ext_grid_active(self):
        ...
        res_ext_grid_active = self.net.res_ext_grid.p_mw.sort_index().to_numpy(copy=True)
    ...
    def get_res_ext_grid_reactive(self):
        ...
        res_ext_grid_reactive = self.net.res_ext_grid.q_mvar.sort_index().to_numpy(copy=True)
    ...
    def get_storage_soc_percent(self):
        ...
        storage_soc_percent = self.net.storage.soc_percent.sort_index().to_numpy(copy=True)
