...
class PlotConfigManager:
    def __init__(self, base_path, yaml_path):
        ...
        self.cfg_default_path = os.path.join(self.parent_path, "yaml", self.yaml_path, "default_config_grid_manage_der.yaml")
        self.cfg_user_path = os.path.join(self.parent_path, "yaml", self.yaml_path, "user_config_grid_manage_der.yaml")
        ...
        self.raw_cfg_user_plot_path = self.main_config["cfg_settings"]["cfg_user_plot_path"]
        self.cfg_user_plot_path = os.path.join(self.parent_path, "yaml", self.yaml_path)
        self.cfg_plot_yaml_path = os.path.join(self.cfg_user_plot_path, self.raw_cfg_user_plot_path)
        self.plot_settings = self.load_plot_configuration(self.cfg_plot_yaml_path)
        
        # Access various plot plot_configuration parameters
        self.basic_plot_params = self.plot_settings['basic_plot'] # 6 LV Grids for every scenario
        self.simple_plot_params = self.plot_settings['simple_plot'] # 6 LV Grids for every scenario (load,sgen)
        self.advanced_plot_params = self.plot_settings['advanced_plot'] # 6 LV Grids for every scenario (load,sgen,storage)
        self.multi_plot_params = self.plot_settings['multi_plot_settings']
        
        # Sets the parameters for the Mini Photovoltaic (MPV) system
        self.mpv_concentration_rate_percent = self.plot_settings['mpv_concentration_rate_percent']
    ...
    def load_plot_configuration(self, cfg_plot_path):
        ...
        plot_settings['multi_plot_settings']['num_scenarios'] = len(self.valid_sb_scenario)
        plot_settings['multi_plot_settings']['num_codes'] = len(self.valid_sb_base_codes)
    ...
    def update_plot_configuration(self):
        self.plot_settings['path']['save_output_plot_path'] = self.plot_settings['path']['parent_path'] + self.plot_settings['path']['output_plot_path']
        self.plot_settings['path']['save_output_latex_path'] = self.plot_settings['path']['parent_path'] + self.plot_settings['path']['output_latex_path']
    ...
    def _set_simbench_episode_settings(self):
        ...
        self.valid_sb_scenario_storage = self.main_config["rawdata"]["valid_sb_scenario_storage"]
        # Set scenario name
    ...
    def get_collection_sizes(self, net, bus_size=1.0, ext_grid_size=1.0, trafo_size=1.0, load_size=1.0,
                             sgen_size=1.0, mpv_sgen_size=1.0, storage_size=1.0, annotation_size=1.0,
                             factor=10.0, distance_factor=200.0):
        """
        ...
        mean_distance_between_buses = sum((net['bus_geodata'].loc[:, ["x", "y"]].max() -
                                           net['bus_geodata'].loc[:, ["x", "y"]].min()).dropna() / distance_factor)
        sizes = {
    ...
    # 4. Hauptfunktionen
    def load_simbench_powergrid(self, code_index, scenario_index, from_disk=False, rotate_plot=False):
        """
    ...
    def advanced_plot(self, net, respect_switches = False, 
                      line_width=1.0, bus_size=1.0, ext_grid_size=1.0, trafo_size=1.0, load_size=1.0, 
                      sgen_size=1.0, mpv_sgen_size=1.0, storage_size=1.0, annotation_size=1.0, 
                      factor=1.0, distance_factor=100.0, mpv_concentration_rate_percent=100.0,
                      scale_size=True, plot_loads=False, plot_sgens=False, plot_mpv_sgens=False,
                      plot_storages=False, plot_annotation=False, 
                      orientation_loads = 1, 
                      orientation_sgens = (25/32), 
                      orientation_mpv_sgens = (10/16), 
                      orientation_storage = (5/4), 
                      bus_color='b', line_color='grey', trafo_color='k',
                      ext_grid_color='k', sgen_color='k', storage_color='k', 
        ...
        lc = plot.create_line_collection(net, plot_lines, color=line_color, linewidths=line_width,
        ...
        if len(net.ext_grid) > 0:
            ext_gridc = plot.create_ext_grid_collection(net,
                                                        size=ext_grid_size, 
                                                        orientation=0,
        ...
        if len(trafo_buses_with_geo_coordinates) > 0:
            tc = plot.create_trafo_collection(net, trafo_buses_with_geo_coordinates,
        ...
        if len(trafo3w_buses_with_geo_coordinates) > 0:
            tc = plot.create_trafo3w_collection(net, trafo3w_buses_with_geo_coordinates,
                                           color=trafo_color)
            collections.append(tc)
    
        if plot_sgens and len(net.sgen):
            sgc = plot.create_sgen_collection(net,
                                              size=sgen_size,
                                              orientation=orientation_sgens*np.pi)
        ...
        if plot_annotation:
            ...
            coords = zip(net.bus_geodata.x.loc[net.bus.index.tolist()].values + 0.10,
                         net.bus_geodata.y.loc[net.bus.index.tolist()].values + 0.10)
            bus_annotation = plot.create_annotation_collection(size=annotation_size,
    ...
    def create_mpv_sgens_to_load_buses_forMPVs(self, net):
        ...
        try:
            ...
            print(f"mpv_concentration_rate_percent: {self.mpv_concentration_rate_percent:>{width}.2f} %")
            print(f"total_loads:                    {total_loads:>{width}d}")
            print(f"num_mpv_influenced_load_buses:  {num_mpv_influenced_load_buses:>{width}d}")
            print(f"num_pv:                         {total_sgens:>{width}d}")
            print(f"mpv_inverter_apparent_power_mva: {self.mpv_inverter_apparent_power_mva:>{width-1}.6f} MW (gamma_1)")
            print(f"mpv_solar_cell_capacity_mw:     {self.mpv_solar_cell_capacity_mw:>{width}.6f} MW (gamma_2)")
            for i in range(num_mpv_influenced_load_buses):
                ...
                if selected_load_bus_id in net.sgen['bus'].values:
                    ...
                    sgen_at_bus = net.sgen[ (net.sgen['bus'] == selected_load_bus_id) & filtered_sgens_name]
                else:
                ...
                pp.create_sgen(net,
                               ...
                               subnet=subnet,
                               phys_type=phys_type)
                print(f"SGenMPV {i} added to grid in Bus Id {selected_load_bus_id}. Total Bus: {len(load_buses)+1}")
            ...
            extract_sgen_mpv_num = net.sgen.name[net.sgen.name.str.contains("SGenMPV")].str.extract('(\d+)$').astype(int)
            ...
            if num_sgen_mpv == num_load:
                print(f"The comparison matches."
    ...
    def cfg_plot_scenarios(self, multi_plot_params, plot_params):
        ...
        for scenario_index in range(num_scenarios):
            ...
            fig, axes = plt.subplots(num_rows, num_cols, figsize=(18, 6))  # Größe anpassen
            ...
            for code_index in range(num_codes):
                ...
                print(f"Plotting: scenario_index={scenario_index}, code_index={code_index}, plot_index={plot_index}, row={row_index}, col={col_index}")
    
                # Laden des Simbench-Codes und des Netzwerks
                sb_code = self.get_simbench_code(code_index,
                                                 scenario_index)
                net, simbench_code = self.load_simbench_powergrid(code_index=code_index, 
                                                                  scenario_index=scenario_index, 
                                                                  rotate_plot=False)
                # net = self._integrate_sgens_into_storage_buses_for_PVBES(net) 
    
                # Entfernen der Line Geodaten
                net.line_geodata.drop(set(net.line_geodata.index) & set(net.line.index), inplace=True)
    ...
    def automatic_plot_save_for_each_code_separate(self, file_path, num_codes, scenario_index, plot_params):
        """
    ...
    def plot_distribution_grid(self, net, choice, ax=None):
        ...
        if choice == "advanced_plot":
            ...
            print("Invalid input. Please choose either 'advanced_plot' or 'simple_plot'.")
            return None
        return ax
    ...
    def simple_script(self, choice="advanced_plot"):
        ...
        """
        ### 
        net, simbench_code = self.load_simbench_powergrid(code_index=0,
                                                          scenario_index=2,
                                                          rotate_plot=False)
        # Add PVs to BES
        net = self._integrate_sgens_into_storage_buses_for_PVBES(net) 
        ...
        self.plot_distribution_grid(net=net, 
                                    choice=choice,
                                    ax=ax)
        return net, ax
