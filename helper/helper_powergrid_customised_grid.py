...
def toy_grid():
    ...
    # Create bus elements
    bus1 = pp.create_bus(net, vn_kv=20., name="MV Busbar", geodata=geodata[0])
    bus2 = pp.create_bus(net, vn_kv=20., name="MV Transformer Bus", geodata=geodata[1])
    bus3 = pp.create_bus(net, vn_kv=0.4, name="LV Transformer Bus", geodata=geodata[2])

    # Create branch elements
    pp.create_line(
        net, from_bus=bus2, to_bus=bus3, length_km=0.1, std_type="NAYY 4x50 SE",
        name="Line 2-3", geodata=[geodata[1], geodata[2]]
    )
    pp.create_transformer(
        net, hv_bus=bus1, lv_bus=bus2, std_type="0.4 MVA 20/0.4 kV",
    ...
    pp.create_storage(
        net, bus=bus3, p_mw=0.030563, max_e_mwh=0.061126, q_mvar=0.030563,
        max_p_mw=0.030563, max_q_mvar=0.030563, sn_mva=0.0306, name="Storage Bus 3"
    )
    pp.create_sgen(net, bus=bus3, p_mw=0.040, q_mvar=0.0, max_p_mw=0.040, name="PV at Bus 3")
...
def toy_two_bus_grid():
    ...
    # Create branch elements
    pp.create_line(
        net, from_bus=bus1, to_bus=bus2, length_km=2.5,
        std_type="NA2XS2Y 1x240 RM/25 12/20 kV", geodata=[geodata[0], geodata[1]]
    )
