...
class PowerGridEvaluator:
    ...
    def __init__(self, powergrid_env):
        ...
        self.sequence_id = self.read_and_increment_sequence_id(self.powergrid_env.sequence_id_path)
        ...
        self.inverter_ctrl_params = self.base_inverter_ctrl_params[self.regulation_standard]
    ...
    def handle_additional_output_file_types(self, file_name_base, dict_data):
        ...
        for output_file_type in self.output_file_types:
            # Construct the output file path
            output_file = os.path.join(self.powergrid_env.output_data_path,
                                       file_name_base + output_file_type)
            print(f"'{output_file}'")
    ...
    @staticmethod
    def write_fig2html(fig, file_name_html="datasets/plot_save/00_res_timeseries_load_plot.html"):
        """
        ...
        html = f'Some html head <img src=\'data:image/png;base64,{encoded}\'> Some more html'
        # html = 'Some html head' + \
    ...
    def res_powergrid_topology_printer(self):
        ...
        advanced_plot_params = self.cfg_plot['main_advanced_plot'] # 6 LV Grids for every scenario (load,sgen,storage)
        figure, ax = plt.subplots(figsize=(18, 6))  # Größe anpassen
        advanced_plot_params['mpv_concentration_rate_percent'] = self.powergrid_env.mpv_concentration_rate_percent 
        plot_params = advanced_plot_params
        obj = PlotConfigManager(self.powergrid_env.helper_path, self.powergrid_env.yaml_path)
    ...
    def plot_power_grid_elements(self, spec_label):
        ...
        total_element_range = [range(total_subplots_per_shape[i], total_subplots_per_shape[i + 1]) 
                               for i in range(len(total_subplots_per_shape) - 1)]
        z=0
        # Schleife durch subplot_shapes und element_range gleichzeitig
        for k_i, (shape, element_idx) in enumerate(zip(base_subplot_shapes, total_element_range), start=1):
            fig, axes = plt.subplots(shape[0], shape[1], figsize=(18.5, 10.5))
            axes = axes.flatten()  # Flatten the array of axes, so we can iterate over it
            fig.suptitle(f'Analysis/Insights "{self.figname}" scenario "{self.powergrid_env.scenario}"', fontsize=12)
            for iter_axex, iter_i in zip(axes, element_idx):
                ...
                if self.keys_list[iter_i] == "sgen.p_mw" or self.keys_list[iter_i] == "sgen.q_mvar":
            ...
            self.save_and_display_figure(fig_name=fig,
                                         plot_file_name=self.plot_names[k_i])
    ...
    def res_timeseries_preparation(self):
        ...
        load_label = [f"load{load_no[i]} bus{loadbus_no[i]}" for i in range(len(self.net.load))]
        sgen_label = [f"{self.net.sgen.type[i]}{sgen_no[i]} bus{sgenbus_no[i]}" for i in range(len(self.net.sgen))]
        bus_label = [f"bus{bus_no[i]}" for i in range(len(self.net.bus))]
        line_labels = [f"{line_name} from_bus{from_bus}_to_bus{to_bus}"
                           for line_name, from_bus, to_bus in
                           zip(self.powergrid_env.net.line.name.values,
                               self.powergrid_env.net.line.from_bus.values,
                               self.powergrid_env.net.line.to_bus.values)]
        trafo_labels = [f"trafo hv_bus{hv_bus}_lv_bus{lv_bus}"
                            for hv_bus, lv_bus in
                            zip(#self.powergrid_env.net.trafo.name.values,
                                self.powergrid_env.net.trafo.hv_bus.values,
                                self.powergrid_env.net.trafo.lv_bus.values)]
        ext_grid_labels = [f"{ext_grid_name} bus{busnr}"
                           for ext_grid_name,busnr in
                           zip(self.powergrid_env.net.ext_grid.name.values,
                               self.powergrid_env.net.ext_grid.bus.values)]
        # Combine spec_labels into a dictionary
        spec_label_base = [load_label, load_label,
                           sgen_label, sgen_label,
                           bus_label, bus_label, bus_label, bus_label,
                           ext_grid_labels, ext_grid_labels,
                           line_labels, line_labels, line_labels,
                           trafo_labels, trafo_labels, trafo_labels,
                           trafo_labels, trafo_labels]
        if self.powergrid_env.mpv_flag:
            mpv_sgen_label = [f"{self.net.sgen.type[i]}{sgen_no[i]} bus{sgenbus_no[i]}" for i in self.powergrid_env.mpv_range_index]
        ...
        if self.powergrid_env.is_storage_scenario:
            ...
            storage_label = [f"storage{storage_no[i]} bus{storage_bus_no[i]}"
                 for i in range(len(self.powergrid_env.net.storage))]
            spec_label_base.extend([storage_label, storage_label, storage_label, storage_label])
    ...
    def res_submain_data_profile(self):
        ...
        axis.xaxis.set_major_locator(plt.matplotlib.dates.HourLocator(byhour=range(0,24),
                                                                    interval=2))
        ...
        self.save_and_display_figure(fig_name=fig9,
    ...
    def plot_element_data_and_confidence_intervals(self, element, axis):
        ...
        Returns:
            None
        """
        axis.plot(self.params[element]['timesteps'], self.params[element]['df_mean'],
                  label=self.params[element]['txt_label'],
                  color=self.params[element]['color'])
        ci_levels = ['ci_lower90', 'ci_upper90', 'ci_lower95',
                     'ci_upper95', 'ci_lower99', 'ci_upper99']
        for ci_lower, ci_upper in zip(ci_levels[::2], ci_levels[1::2]):
            axis.fill_between(self.params[element]['timesteps'],
    ...
    def _submain_analyze_data(self, element):
        ...
        if element == "load":
            # "load"
            load_data = self.powergrid_env.log_variables["res_load.p_mw"] # load_active [MW] 
            ...
            # "storage"
            storage_data = self.powergrid_env.log_variables["storage.p_mw"] # storage_active
            data = pd.DataFrame(storage_data) if isinstance(storage_data, list) else storage_data
        ...
        ci_lower99, ci_upper99= self._calculate_confidence_interval(data_frame,
                                                                    alpha=0.01)
        ci_lower95, ci_upper95= self._calculate_confidence_interval(data_frame,
                                                                    alpha=0.05)
        ci_lower90, ci_upper90= self._calculate_confidence_interval(data_frame,
                                                                    alpha=0.10)

        if self.time_option == "datetime":
            # Create TimeSteps datetime axis
            customdate = datetime.datetime(2023, 1, 22, 00, 00) #datetime.datetime.now()
            timesteps = np.array([customdate + \
    ...
    def _calculate_zone_dataframe(self, data_frame, mappings):
        ...
        zone_mean = {zone: {} for zone in zones} # Create empty dictionary for each zone
        ...
        for col in data_frame.columns:
            ...
            # Loop over all rows
            for i in range(self.powergrid_env.timesteps_per_day):
                mean_values.append(data_frame[col][i::self.powergrid_env.timesteps_per_day].mean())
    ...
    def _calculate_confidence_interval(self, data_frame, alpha = 0.05):
        ...
        margin_of_error = t_critical * std / np.sqrt(self.powergrid_env.timesteps_per_day)
    ...
    def res_key_performances_index_preparation(self):
        ...
        bus_vm_mv = self.powergrid_env.log_variables["res_bus.vm_pu"] # bus_voltage [p.u] 
        bus_vm_lv = self.buses_vm_ts_lv.dropna(axis=1)
        assert bus_vm_mv.shape[1]-1 == bus_vm_lv.shape[1], "Da nur ein Trafo vorliegt -1"
        self.kpi_raw = {
        ...
        bus_vm_ts_stats = self.get_ts_statistics(data_frame=self.kpi_raw["buses_vm_ts_lv"])
        bus_vm_scalar_stats = self.get_scalar_vr_statistics(self.kpi_raw["buses_vm_ts_lv"])
        lines_vm_stats = self.calculate_ts_data_stats_kpi1(self.kpi_raw["lines_ts_data"])
        lines_ts_stats = self.get_ts_statistics(data_frame=self.kpi_raw["lines_ts_data"])
    ...
    def _calculate_vm_violations_kpi1(self):
        ...
        self.kpi_vm_violations = {
            "vm_lv_mean_avg": 
                self.kpi_raw["buses_vm_ts_max_min_avg_bus_node_statistics"]["average"].mean(),
            "vm_lv_mean_min": 
                self.kpi_raw["buses_vm_ts_max_min_avg_bus_node_statistics"]["average"].min(),
            "vm_lv_mean_max": 
                self.kpi_raw["buses_vm_ts_max_min_avg_bus_node_statistics"]["average"].max(),
            "vm_lv_mean_std": 
                self.kpi_raw["buses_vm_ts_max_min_avg_bus_node_statistics"]["average"].std(),
            "vm_lv_highest_max": 
                self.kpi_raw["buses_vm_ts_max_min_avg_bus_node_statistics"]["maximum"].max(),
            "vm_lv_lowest_min": 
                self.kpi_raw["buses_vm_ts_max_min_avg_bus_node_statistics"]["minimum"].min(),
            'vm_num_violations_dam': 
                f"({self.vm_num_violations_damage},"
                f"min:{self.vm_limits['min_damage_vm_pu'][1]},"
                f"max:{self.vm_limits['max_damage_vm_pu'][1]})",
            'vm_num_violations_cri': 
                f"({self.vm_num_violations_critic},"
    ...
    def _calculate_trafo_and_lines_violations_kpi2(self):
        ...
        self.kpi_transformer_and_lines_violations = {
            'transformer_num_violations_dam':
                f"({self.transformer_num_violations_dam},"
                f"'{self.transformer_max}%')",
            "transformer_loading_avg":
                self.kpi_raw["trafo_node_statistics"].loc["average"][0],
            "transformer_loading_max":
                self.kpi_raw["trafo_node_statistics"].loc["maximum"][0],
            "transformer_loading_min":
                self.kpi_raw["trafo_node_statistics"].loc["minimum"][0],
            "transformer_loading_std":
                self.kpi_raw["trafo_node_statistics"].loc["std_dev"][0],
            'lines_num_violations_dam_sum':
                f"({self.lines_num_violations_dam.sum()},"
                f"'{self.lines_max}%'",
            "lines_loading_mean_avg":
                self.kpi_raw["lines_vm_ts_edges_statistics"].loc["average"].mean(),
            "lines_loading_mean_max":
                self.kpi_raw["lines_vm_ts_edges_statistics"].loc["maximum"].mean(),
            "lines_loading_mean_min":
                self.kpi_raw["lines_vm_ts_edges_statistics"].loc["minimum"].mean(),
            "lines_loading_mean_std":
                self.kpi_raw["lines_vm_ts_edges_statistics"].loc["std_dev"].mean(),
            "lines_loading_highest_max":
                self.kpi_raw["lines_vm_ts_edges_statistics"].loc["maximum"].max(),
            "lines_loading_lowest_min":
                self.kpi_raw["lines_vm_ts_edges_statistics"].loc["minimum"].min(),
        }
    ...
    def _calculate_power_losses_and_power_balance_kpi3(self):
        ...
        transformer_losses_p_mw = log_variables["res_trafo.pl_mw"] # trafo_active power losses [MW]
        transformer_losses_q_mvar = log_variables["res_trafo.ql_mvar"] # trafo_reactive power losses [MVar] 
        # Power supplied from external grid and loads
        res_ext_grid_p_mw = log_variables["res_ext_grid.p_mw"] # ext_grid_active_supply [MW]
        res_ext_grid_q_mvar = log_variables["res_ext_grid.q_mvar"] # ext_grid_reactive_supply [MVar]
        ...
        total_power_loss_q_mvar = total_line_losses_q_mvar + total_transformer_losses_q_mvar
        ...
        if not self.powergrid_env.is_storage_scenario:
            ...
            total_of_storage_discharge_power_loss = (
                log_variables["storage.discharge_power_loss"] # storage_discharge_power_loss
                .sum()
                .sum()
                )
        ...
        micro_grid_q_mvar = (total_of_sgen_q_mvar - total_res_load_q_mvar
                             - total_of_storage_q_mvar - total_power_loss_q_mvar)
        micro_grid_p_mw = (total_of_sgen_p_mw - total_res_load_p_mw
                           - total_of_storage_p_mw - total_power_loss_p_mw)
        ...
        if self.powergrid_env.mpv_flag:
            ...
            mpvsgen_p_mw = log_variables["sgen.p_mw"].loc[:, mask_mpv] # MPV pv_active [MW]
            mpvsgen_q_mvar = log_variables["sgen.q_mvar"].loc[:, mask_mpv] # MPV pv_reactive [MVar]
            ...
            pvsgen_p_mw = log_variables["sgen.p_mw"].loc[:, mask_pv] # PV pv_active [MW]
            pvsgen_q_mvar = log_variables["sgen.q_mvar"].loc[:, mask_pv] # PV pv_reactive [MVar]
    ...
    def _calculate_storage_utilization_kpi4(self):
        ...
        storages_soc_ts_data = log_variables["storage.soc_percent"] # storage_soc_percent
        storages_soc_ts_nodes_stats= self.calculate_ts_data_stats_kpi1(storages_soc_ts_data)
    ...
    def plot_vm_min_max_buses_kpi1(self, plot_name):
        ...
        ln1 = ax1.plot(data_frame.index, data_frame['maximum'],
                       color=color[1], linestyle='-', label='Maximum Voltage Magnitude')
        ln2 = ax1.plot(data_frame.index, data_frame['average'],
                       color=color[0], linestyle='-', label='Average Voltage Magnitude')
        ln3 = ax1.plot(data_frame.index, data_frame['minimum'],
                       color=color[4], linestyle='-', label='Minimum Voltage Magnitude')
        ln4 = ax2.plot(data_frame.index, data_frame['maximum_idx'],
                       color=color[1], linestyle=':', label='Maximum Bus Number')
        ln5 = ax2.plot(data_frame.index, data_frame['minimum_idx'],
                       color=color[4], linestyle=':', label='Minimum Bus Number')
    ...
    def plot_lines_and_transformer_overloading(self, plot_name):
        ...
        fig.suptitle('Transformer and Lines Analysis')  # Titel für die gesamte Figur setzen
    ...
    def plot_transformer_data(self, ax1, ax2):
        ...
        """
        ax2.text(0.5, 0.5, stats_trafo, horizontalalignment='center',
    ...
    def load_ctrl_mode_and_technical_params(self):
        ...
        ctrl_mode_param_keys_1 = ["v_nom","v_nom_net","v_1","v_2","v_3","v_4","v_5"]
    ...
    def compute_voltage_limits_kpi1(self):
        ...
        """
        max_crit_vm_pu = self.net.bus.min_vm_pu.values+0.015*2*self.net.bus.min_vm_pu.values
        min_crit_vm_pu = self.net.bus.max_vm_pu.values-0.015*1*self.net.bus.max_vm_pu.values
        # Speichern der Variablen in einem DataFrame
        df_max_damage = pd.DataFrame(self.net.bus.max_vm_pu.values, columns=['max_damage_vm_pu'])
        df_min_damage = pd.DataFrame(self.net.bus.min_vm_pu.values, columns=['min_damage_vm_pu'])
        ...
        df_vm_limits = pd.concat([df_max_damage, df_min_damage,
                                   df_max_critic, df_min_critic],
                                  axis=1)
        ...
        limits_vars= {'v_max_damage': v_max_damage, # 'max_damage_vm_pu'
                      'v_min_damage': v_min_damage, # 'min_damage_vm_pu'
                      'v_max_critic': v_max_critic, # 'max_critic_vm_pu'
                      'v_min_critic': v_min_critic} # 'min_critic_vm_pu'
    ...
    def assign_kpi1(self):
        ...
        self.sim_arguments = {
                ...
                "storage_p_ctrl": self.powergrid_env.args["control_modes"]["storage_p_ctrl"],
                "storage_q_ctrl": self.powergrid_env.args["control_modes"]["storage_q_ctrl"],
        ...
        self.kpi_key = {
            ...
            "transformer_loading_avg": 
                self.kpi_transformer_and_lines_violations["transformer_loading_avg"],
            ...
            "grid_loss_p_mw": 
                self.kpi_power_losses_and_power_balance["grid_loss_p_mw"],
            "total_of_sgen_p_mw": 
                self.kpi_power_losses_and_power_balance["total_of_sgen_p_mw"],
            "total_of_sgen_q_mvar": 
                self.kpi_power_losses_and_power_balance["total_of_sgen_q_mvar"],
            "total_res_ext_grid_p_mw": 
                self.kpi_power_losses_and_power_balance["total_res_ext_grid_p_mw"],
        }
        ...
        dict_data = {
            'kpi_key': 
                pd.DataFrame(self.kpi_key, index=[0]).transpose(),
            'kpi_vm_violations': 
                pd.DataFrame(self.kpi_vm_violations, index=[0]).transpose(),
            'kpi_trafo_and_lines_violations': 
                pd.DataFrame(self.kpi_transformer_and_lines_violations, index=[0]).transpose(),
            'kpi_power_loss_and_balance': 
                pd.DataFrame(self.kpi_power_losses_and_power_balance, index=[0]).transpose(),
            'sim_arguments': 
                pd.DataFrame(self.sim_arguments, index=[0]).transpose(),
            'buses_vm_ts_nodes_statistics': 
                pd.DataFrame(self.kpi_raw["buses_vm_ts_nodes_statistics"]),
            'buses_vm_ts_node_statistics': 
                pd.DataFrame(self.kpi_raw["buses_vm_ts_max_min_avg_bus_node_statistics"])
        }
        ...
        self.handle_additional_output_file_types(file_name_base=self.file_name[0],
                                                 dict_data=dict_data)
    ...
    def get_ts_statistics(data_frame: pd.DataFrame) -> pd.DataFrame:
        ...
        ts_statistics = pd.concat([df_ts_max_values, df_ts_max_index,
                                   df_ts_min_values, df_ts_min_index,
                                   df_ts_avg_values], axis=1)
    ...
    def get_lines_stats_df(self):
        ...
        for column in self.kpi_raw['lines_ts_data'].columns:
            stats_df[column] = [round(self.kpi_raw['lines_ts_data'][column].min(), 2),
        ...
        stats_df['critic'] = [round(stats_df.loc['min'].min(), 2),
        ...
        stats_df['idx'] = [stats_df.loc['min'].idxmin(),
                           stats_df.loc['max'].idxmax(),
                           None, # There is no corresponding index for the average
                           None] # There is no corresponding index for the standard deviation
        return stats_df
