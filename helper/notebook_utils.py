...
def combine_and_save_dataframes(
    dfs, DIRECTORY_PATH, OUTPUT_CSV_FILE_PATH="combined_solution.csv"
):
    combined_df = pd.concat(dfs, ignore_index=True)  # Combine DataFrames
    combined_df.to_csv(
        f"{DIRECTORY_PATH}{OUTPUT_CSV_FILE_PATH}", index=False
    )  # Save to CSV
...
def extract_numeric_data(df, capacities):
    for column in ["transformer_num_violations_dam", "lines_num_violations_dam_sum"]:
        for capacity in capacities:
            ...
            df[f"{column}_limit"] = (
                df[capacity][column].str.extract(r"'([\d.]+)%'").astype(float)
            )
            df[capacity][column] = (
                df[capacity][column].str.extract(r"(\d+)").astype(int)
            )
    return df
    ...
    for capacity in capacities:
        ...
        if capacity in reset_group_dfs:
            ...
            for key in group_df["kpi_key"].iloc[0].keys():
                reset_group_dfs[capacity][key] = group_df["kpi_key"].apply(
                    lambda x: x.get(key)
                )
...
def preprocess_data(directory_path, output_csv_file_path):
    ...
    combined_df = combine_and_save_dataframes(
        loaded_dfs, directory_path, output_csv_file_path
    )
...
def calculate_global_limits(
    reset_group_dfs, capacities, kpi_param="transformer_loading_avg"
):
    """
    ...
    for capacity in capacities:
        ...
        x_min, x_max = (
            df["MPV_CONCENTRATION_RATE_PERCENT"].min(),
            df["MPV_CONCENTRATION_RATE_PERCENT"].max(),
        )
...
def inches_to_cm(inches):
    ...
    return (
        [value * 2.54 for value in inches]
        if isinstance(inches, (list, tuple))
        else inches * 2.54
    )
...
def create_combined_plot(
    ...
    for i, capacity in enumerate(CAPACITIES):
        ...
        for power, style in marker_styles.items():
            ...
            axes[row_index, i].plot(
                df_filtered["MPV_CONCENTRATION_RATE_PERCENT"],
                df_filtered[kpi_param],
                marker_style,
...
def calc_and_round_setpoints(
    data_list,
    num_setpoints=5,
    decimal_places=3,
    small_step_indices=None,
    middle_step_indices=None,
    small_middle_indices=None,
    small_step_sizes=[0.01, 1, 2, 5],
    middle_step_sizes=[5, 10, 15, 20],
    small_middle_sizes=[3, 6, 9, 12],
    large_step_sizes=[25, 50, 75],
    repetitions=4,
):
    # Default settings
...
plot_configurations = [
    {
        ...
        "dimensions_cm": (
            21,
            14.85 - 3,
        ),  # Width, Height (caption space already subtracted)
...
def plot_analysis(
    reset_group_dfs, CAPACITIES, file_name, base_path="output/notebook_cs_1/"
):
    # Plot creation
    for config in plot_configurations:
        fig_width, fig_height = cm_to_inches(config["dimensions_cm"][0]), cm_to_inches(
            config["dimensions_cm"][1]
        )
        hspace, wspace = config["spacing"]
        print(
            f"Output: {config['page']}\nCreating plot with width: {fig_width} inches, height: {fig_height} inches"
        )
        ...
        globals_xy = [
            calculate_global_limits(reset_group_dfs, CAPACITIES, kpi_param=kpi)
            for kpi in config["kpi_params"]
        ]
        ...
        for row_index, kpi_param in enumerate(config["kpi_params"]):
            create_combined_plot(
                ...
                row_index,
                kpi_param,
                config["y_labels"][row_index],
                *globals_xy[row_index],
                config["marker_styles"],
            )
...
def utils_scan_folders(path, start_name_criteria):
    ...
    for name in os.listdir(path):
        ...
        if os.path.isdir(full_path) and any(
            name.startswith(start_name) for start_name in start_name_criteria
        ):
