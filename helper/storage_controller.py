...
class StorageController(Controller):
    ...
    def __init__(self,
        ...
        self.init_storage_parameters(net=net,
    ...
    def init_der_control_mode_settings(self, net, der_control_mode, storage_q_control_mode,
    ...
    def generate_error(self, value):
        ...
        if self.add_vm_pu_noise:
            error = np.random.normal(loc=self.loc, 
                                     scale=self.error_std_dev)
            return value * error
        else:
            error = 0
            return value * error
    ...
    def calculate_param_with_error(self, net, elm_name, sid, param_name, default_val):
        ...
        initial_value = self.get_initial_value(net, elm_name, sid, param_name, default_val)
    ...
    def _load_storage_settings(self, net):
        ...
        self.min_p_mw = (net.storage.at[self.sid, "min_p_mw"]
                         if "min_p_mw" in net.storage.keys() else -self.max_e_mwh/5)
        self.max_p_mw = (net.storage.at[self.sid, "sn_mva"]
                         if "sn_mva" in net.storage.keys() else self.max_e_mwh/5)
        # Maximum/Minimum reactive power (Q) injection/absorption
        # Minimum (discharging) reactive power
        self.min_q_mvar = (net.storage.at[self.sid, "min_q_mvar"]
                           if "min_q_mvar" in net.storage.keys() else self.min_q_mvar)
        # Maximum (charging) reactive power
        self.max_q_mvar = (net.storage.at[self.sid, "max_q_mvar"]
                           if "max_q_mvar" in net.storage.keys() else self.max_q_mvar)
    ...
    def write_to_net(self, net):
        ...
        """
        if self.soc_percent >= self.soc_percent_discharge_limit and self.soc_percent <= self.soc_percent_charge_limit:
    ...
    def update_and_monitor_storage_power(self, desired_p_mw, time):
        ...
        if self.norm_mom_p_mw > self.norm_p_mw_lower_threshold:
            self.monitor_storage_control(time=time,
                                         mom_bat_p_mw=self.mom_bat_p_mw)
        if self.q_mvar > 0:
            self.monitor_storage_control(time=time,
                                         mom_bat_p_mw=self.mom_bat_p_mw)
    ...
    def adjust_storage_power_output(self, desired_p_mw):
        ...
        elif (self.soc_percent > self.soc_percent_discharge_limit) and (desired_p_mw <= 0):
    ...
    def monitor_storage_control(self, time, mom_bat_p_mw):
        ...
        # self.v_pcc = v_pcc
        print("self.sid:", self.sid, " time:", time, 
              "p_mw:", self.p_mw, 
              "mom_bat_p_mw:", mom_bat_p_mw,
              "self.sid:", self.sid, "soc_percent:", self.soc_percent)
        ...
        elif (self.soc_percent <= self.soc_percent_discharge_limit) and (mom_bat_p_mw < 0):
    ...
    def run_battery_q_control(self, net, storage_q_control_mode):
        ...
        if storage_q_control_mode == "voltage_reactive_power_ctrl":
            ...
            self.voltage_reactive_power_control_mode(net=net,
                                                     mom_p_mw=self.mom_bat_p_mw)
    ...
    def constant_power_factor_active_power_ctrl(self, mom_p_mw):
        ...
        self.norm_q_mvar = max(self.norm_min_q_mvar, min(q_norm_from_cos_phi, self.norm_max_q_mvar))
        # Calculate normalized active power from apparent and reactive power
        self.norm_p_mw = min(self.norm_mom_p_mw, np.sqrt(self.norm_sn_mva ** 2 - self.norm_q_mvar ** 2))
    ...
    def rbc_pvbes_decentralized_sc_ctrl(self, net, p_mw = 0):
        ...
        elif (self.soc_percent > self.soc_percent_discharge_limit) and (residual_power < 0):
            # negative discharging
            p_discharge_mw = self.min_p_mw if load_power < self.min_p_mw else -load_power
    ...
    def rbc_pvbes_distributed_sc_ctrl(self, net, p_mw = 0):
        ...
        if (self.soc_percent < self.soc_percent_charge_limit) and residual_power > 0:
            # positive charging
            p_charge_mw = self.max_p_mw if residual_power > self.max_p_mw else residual_power
            p_mw = p_charge_mw
        # (II) Battery Discharging Block:
        elif self.soc_percent > self.soc_percent_discharge_limit and residual_power <= 0.0:
            # negative discharging
            p_discharge_mw = self.min_p_mw if residual_power < self.min_p_mw else residual_power
            p_mw = p_discharge_mw
        else:
            p_mw = 0
        net.storage.at[self.sid, "p_mw"] = p_mw
    ...
    def rbc_pvbes_distributed_sc_dnc_ctrl(self,net, time, p_mw = 0):
        ...
        # (II) Battery Discharging Block:
              ...
              residual_power <= 0 and flag_time):
            # negative discharging
            p_discharge_mw = self.min_p_mw if residual_power < self.min_p_mw else residual_power
            p_mw = p_discharge_mw
        else:
    ...
    def rbc_bes_dnc_strategy_ctrl(self, net, time, p_mw=0):
        ...
        is_discharging_time = not is_charging_time # Discharging from 6:00 p.m. to 6:00 a.m.
    
        # (I) Battery Charging Block:
        if is_charging_time and self.soc_percent < self.soc_percent_charge_limit:
            # positive charging
            p_charge_mw = min(self.max_p_mw, net.sgen.p_mw.sum()) # Charge with available solar power or max capacity
            p_mw = p_charge_mw
        # (II) Battery Discharging Block:
        elif is_discharging_time and self.soc_percent > self.soc_percent_discharge_limit:
            # negative discharging
            p_discharge_mw = max(self.min_p_mw, -net.load.p_mw.sum()) # Discharge to meet load or min capacity
            p_mw = p_discharge_mw
        else:
            # Idle state
            p_mw = 0
