...
class StorageControllerTest(StorageController):
    ...
    def __init__(self, *args, **kwargs):
        ...
        step_size = 0.001
        num_steps = 21
        active_power_range = np.linspace(
            self.min_p_mw, self.max_p_mw + 2 * step_size, num_steps
        )
        num_steps = 21
        ...
        for v_meas in voltage_range:  # Iterate over voltage range
            ...
            for mom_p_mw in active_power_range:  # Iterate over power range
                ...
                self.update_and_monitor_storage_power(
                    desired_p_mw=self.mom_bat_p_mw, time=iter_time
                )
                self.run_battery_q_control(
                    net=self.net, storage_q_control_mode=self.storage_q_control_mode
                )
                ...
                data_dict = {
                    f"self.{attr}": [getattr(self, attr)] for attr in attributes_storage
                }
    ...
    def run_rule_based_control_vde_tests(self, net):
        ...
        for mode in ["deadband", "base"]:
            ...
            results_v = self._iterate_over_voltage_and_power(
                voltage_range, active_power_range, attributes_v, net
            )
            ...
            print(
                "self.pv_regulation_standard_mode:",
        ...
        for pv_control_mode, data in control_modes.items():
            ...
            for name, min_cos_phi_value, test_mode in zip(
                data["names"], data["min_cos_phi_values"], data["test_mode"]
            ):
                self.min_cos_phi = min_cos_phi_value
                if test_mode:
                    self.load_power_factor_settings(test_mode=test_mode)
                    self.load_control_rule_settings(net=net, element_type="storage")
    ...
    def sub_test_power_factor_control_mode(
        self, active_power_range, attributes_p, pv_control_mode
    ):
        """
        ...
        """
        results_p = self._iterate_over_power(
            active_power_range, attributes_p, pv_control_mode
        )
    ...
    def _iterate_over_voltage_and_power(
        self, voltage_range, active_power_range, attributes, net
    ):
    ...
    def _create_3d_draw(
        ...
        cbar = matplotlib.colorbar.ColorbarBase(
            cax, cmap=cmap_base, norm=normalize, orientation="vertical"
        )
        # Set colorbar label
    ...
    def draw_pqv_diagram(self, results):
        ...
        # P
        norm_p = lambda p: (p - self.norm_min_p_mw) / (
            self.norm_max_p_mw - self.norm_min_p_mw
        )
    ...
    def draw_qv_diagram(self, results):
        ...
        # results = results_v
        ax1.plot(
            results["self.vm_pu_meas"],
            results["self.norm_q_mvar"],
            label="Q(V) with constraint",
        )
        ax1.plot(
            results["self.vm_pu_meas"], results["self.raw_norm_q_mvar"], label="Q(V)"
        )
        ax1.legend()
        ...
        ax1.set_ylim(
            self.norm_min_q_mvar * self.c_axis_factor,
        ...
        yticklabels = [
            (
                f"{tick:.3f}=$Q_{{max}} (under excited)$"
                if tick == self.norm_max_q_mvar
                else (
                    f"{tick:.3f}=$Q_{{max}} (over excited)$"
                    if tick == self.norm_min_q_mvar
                    else f"{tick:.3f}"
                )
            )
            for tick in yticks
        ...
        yticklabels_cos = [
            round(np.cos(np.arctan(tick)), self.c_precision) for tick in yticks
        ]
        ...
        if self.pv_regulation_standard_mode == "deadband":
            ...
            y_data = (
                [self.norm_max_q_mvar] + [self.nomi_q_mvar] * 3 + [self.norm_min_q_mvar]
            )
    ...
    def draw_pq_diagram(self, results):
        ...
        plt.plot(
            -results["self.norm_p_mw"], results["self.norm_q_mvar"], label="PQ points"
        )
        ...
        circle2 = Circle(
            (0, 0),
            self.reactive_power_performance_limit,
            color="grey",
            linestyle="--",
            fill=True,
            label="Reactive power performance limit",
            alpha=0.2,
        )
        ...
        triangle = Polygon(
            triangle_coordinates, edgecolor="red", facecolor="green", alpha=0.1
        )
    ...
    def draw_cos_phi_p_diagram(self, results):
        ...
        # Create the PQ diagram
        plt.plot(
            results["self.norm_mom_p_mw"],
            results["self.mom_cos_phi"],
            label=r"$\cos \gamma$",
        )
        # Add text to the vertices of the triangle
        ...
        plt.plot(
            [0, 1], [self.min_cos_phi, self.min_cos_phi], color="black", linestyle="--"
        )
